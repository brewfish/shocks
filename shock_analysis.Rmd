...
test_list <- agg_fisheries_list %>% 
  map(., ~detect_shocks(., cooks_d_threshold = 0.3, n_baseline_yrs = 7)) # Not working
...
# Amount of biomass per species, per country, per year, resulting in diversity index
```

Plot:
```{r}
# Aggregated catch time series with points colored by number of species

test <- agg_fisheries_list[112:119]
(shocks_vs_n_spp_ts <- map(test, 
                           ~ggplot(., aes(x = year, y = sum_quantity, color = div)) + 
                             geom_hline(yintercept = 0, color = "darkgray") +
                             geom_vline(data = subset(., is_shock == TRUE & abs_magnitude < 0), xintercept = subset(., is_shock == TRUE & abs_magnitude < 0)$year, color = "red", linetype = "dashed") +
                             #geom_vline(data = subset(., is_shock == TRUE & abs_magnitude > 0), xintercept = subset(., is_shock == TRUE & abs_magnitude > 0)$year, color = "darkgreen") +
                             geom_path() +
                             geom_point(size = 2, pch = 16) +
                             scale_color_gradient(low = "gold", high = "red", limits = c(1,60)) +
...
pdf(file = here("output", "shocks_vs_n_spp_ts.pdf"), width = 10)
