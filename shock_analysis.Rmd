...
agg_production_plots <- map(agg_production_list, ~ggplot(., aes(x = year, y = sum_quantity, color = sector)) + 
               geom_hline(yintercept = 0, color = "darkgray", linetype = "dashed") +
               geom_line() +
               geom_point() +
               scale_color_manual(values = c("darkred", "darkblue"), labels = c("Aquaculture", "Capture")) +
...
pdf("fig_agg_production_bycountry.pdf", paper = "a4r")
agg_production_plots
dev.off()

# Still wanna do: plot 2 figs per page, apply consistent y-axis, apply x-axis that always starts at 0
```
