...
Import B/B~msy~ time series from the RAM database, selecting the `TBdivTBmsy` variable (relative to total biomass), or `SSBdivSSBmsy` variable (relative to spawning stock biomass) if the former is absent. Following the methods of much published literature, including Pinksy et al. [-@pinsky_fishing_2015], I will define a collapse as a year where B < 20% of B~msy~.
...
collapses <- timeseries_values_views  %>% 
  select(stockid, stocklong, year, TBdivTBmsy, SSBdivSSBmsy) %>% 
  filter(year >= 1980) %>%  # Trim time series to 1980 onwards
  mutate(ram_bdivbmsy = ifelse(!is.na(TBdivTBmsy), TBdivTBmsy, SSBdivSSBmsy)) %>% 
  filter(ram_bdivbmsy < 0.20) %>% # Selecting collapses only
  group_by(stockid, stocklong, tmp = c(0, cumsum(diff(year) != 1))) %>% 
...
ggplot(collapses, aes(x = fct_infreq(major_group))) +
  geom_bar() +
  geom_text(stat = "count", aes(label = paste("n =", ..count..)), y = -0.015) +
  labs(title = "Number of stock collapses per major taxonomic group", x = "Major group", y = "Count") + 
  theme_pubr()
```
