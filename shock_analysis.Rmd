...
The shock detection method presented above isn't without flaws. Here I will cross-reference the shocks detected 
...
diversity <- production %>%
  ...
  mutate(div = diversity(.[,c(-1:-6)], index = "invsimpson"),
         log_div = log(div)) %>% 
  select(country, country_name, year, div, log_div)
...
agg_fisheries_list <- agg_fisheries_list %>% 
  ...
  map(., ~left_join(., diversity[,c("country_name","year","div", "log_div")], by = c("country_name", "year")))
...
diversification_ts <- map(agg_fisheries_list, 
                           ~ggplot(., aes(x = year, y = sum_quantity, color = log_div)) + 
...
agg_fisheries_list %>% 
  bind_rows() %>% 
  filter(is_shock == TRUE) %>% 
  mutate(log_recovery = log(recovery)) %>% 
  ggscatter(., x = "log_div", y = "recovery")

agg_fisheries_list %>% 
  bind_rows() %>% 
  filter(is.na(is_shock)) %>% 
  gghistogram(., x = "log_div") 

# Number of shocks vs number of spp fished
...
group_shocks <- group_production_list %>% 
  ...
  summarize(ts_length = last(year) - first(year),
         n_shocks = length(which(is_shock==TRUE)),
