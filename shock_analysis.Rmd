...
countries <- agg_production %>% 
  ...
  summarise(min_yr = min(year), # n = 247 countries
            max_yr = max(year),
            ts_len = max_yr-min_yr) %>% 
  ungroup(country, country_name) %>% 
  filter(ts_len >= 30) %>%
  pull(country) # n = 212 countries

# Select only capture fisheries data from the countries indentified above
agg_fisheries <- subset(agg_production, sector == "capture" & country %in% countries)
```

## Total number of shocks across a range of Cook's D values
