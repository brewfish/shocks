---
title: "Cottrell et al. (2019)'s shock analysis, updated"
...
cropsfull<-read.csv(unzip("Production_Crops_E_All_Data_(Norm).zip"))
names(cropsfull)
...
crops<-read.csv("croptotals.csv", header = T, encoding = "latin1")
...
for (i in 1:length(levels(aggcrops$Country))){ #ERROR
  values<-subset(aggcrops$Value2, aggcrops$Country==levels(aggcrops$Country)[i])
...
livestockfull<-read.csv(unzip("Production_LivestockPrimary_E_All_Data_(Norm).zip"))
names(livestockfull)
...
livestock<-read.csv("livestocktotals.csv", header = T, encoding = "latin1")
...
fisheries<-read.csv("fisheries - Watson adjusted landings 1950-2014 new.csv")
summary(fisheries)
...
write.csv(fisheries, file="fisheries - Watson adjusted landings 1950-2014 new2.csv", row.names = F)

fisheries <- read.csv("fisheries - Watson adjusted landings 1950-2014 new2.csv", header=T, encoding = "latin1")
...
freshfish <- read.csv("freshwater fisheries landings 1950 - 2015 tidy.csv", header = T, encoding = "latin1")
...
#aggregate production
  ...
  for (i in 1:length(levels(aggfreshfisheries$Country))){
    values<-subset(aggfreshfisheries$Value, aggfreshfisheries$Country==levels(aggfreshfisheries$Country)[i])
...
levels(freshfish$Country)[levels(freshfish$Country)=="C?te d'Ivoire"]<-"Cote d'Ivoire"
freshfish <- freshfish[freshfish$Country!="Saint Helena",]
...
inland$Codes[inland$Country=="Réunion"] <- "REU"
...
inland <- read.csv("inland aquaculture 1950 -2015 tidy.csv", header=T, encoding = "latin1")
...
marine$Codes[marine$Country=="Réunion"] <- "REU"
...
marine <- read.csv("Marine aquaculture production 1950 -2013 tidy.csv", header=TRUE, encoding = "latin1")
...
```{r}
crops<-read.csv("Crop Production 1961-2013.csv", header=T, encoding = "latin1")
...
livestock <- read.csv("Livestock Production 1961-2013.csv", header=T, encoding = "latin1")
...
marinefisheries <- read.csv("Marine fisheries Production 1950-2014.csv", header = T, encoding = "latin1")
...
inlandfisheries<- read.csv("Inland fisheries Production 1950-2014.csv", header = T, encoding = "latin1")
...
inlandaqua<- read.csv("Inland aquaculture Production 1950-2014.csv", header=T, encoding = "latin1")
...
marineaqua <- read.csv("Marine aquaculture production 1950 -2015.csv", header=T, encoding = "latin1")
...
## This section is where aggfisheries is created!
...
aggfisheries <- read.csv("aggfisheries.csv", header=T, encoding = "latin1")
...
aggmarineaqua2$Codes[aggmarineaqua2$Country=="Réunion"]<-"REU"
...
agginlandaqua2$Codes[agginlandaqua2$Country=="Réunion"]<-"REU"
...
aggaqua <- read.csv("aggaqua.csv", header=T, encoding = "latin1")
...
W_Africa <- c("Benin", "Burkina Faso", "Cabo Verde", "C?te d'Ivoire", "Gambia", "Ghana", "Guinea", "Guinea-Bissau", "Liberia", "Mali", "Mauritania", "Niger", "Nigeria", "Senegal", "Sierra Leone", "Togo")
...
Shocks.df <- cbind.data.frame(D, sumofshocksacrossD) #use this if you didnt calc shocks.df above with the otehr sectors
...
shocks$Codes[shocks$Country=="R?union"] <- "REU"
...
# shockdrivers<-shockdrivers[!(shockdrivers$Sector=="Aquaculture" & shockdrivers$Country=="Réunion"),]
...
N_Africa <- c("Algeria", "Egypt", "Libya", "Morocco", "Sudan", "Tunisia")
E_Africa <- c("Burundi", "Comoros", "Djibouti", "Eritrea", "Ethiopia", "Kenya", "Madagascar", "Malawi", "Mauritius", "Mozambique", "Rwanda", "Seychelles", "Somalia", "South Sudan", "Uganda","Tanzania", "Zambia", "Zimbabwe","United Republic of Tanzania" , "Tanzania, United Rep. of", "Zanzibar", "R?union")
M_Africa <- c("Angola", "Cameroon", "Central African Republic", "Chad", "Congo", "Dem. Rep. Congo", "Equatorial Guinea", "Gabon", "Sao Tome and Principe", "St Helena", "Congo, Dem. Rep. of the", "Democratic Republic of the Congo" )
S_Africa <- c("Botswana", "Lesotho", "Namibia", "South Africa", "Swaziland")
W_Africa <- c("Benin", "Burkina Faso", "Cabo Verde", "C?te d'Ivoire", "Gambia", "Ghana", "Guinea", "Guinea-Bissau", "Liberia", "Mali", "Mauritania", "Niger", "Nigeria", "Senegal", "Sierra Leone", "Togo", "Western Sahara")
...
shockcategories$Value[shockcategories$Var2=="Aquaculture"]<- sum(shockcategories$Freq[shockcategories$Var2=="Aquaculture"])
shockcategories$Value[shockcategories$Var2=="Crops"]<- sum(shockcategories$Freq[shockcategories$Var2=="Crops"])
shockcategories$Value[shockcategories$Var2=="Livestock"]<- sum(shockcategories$Freq[shockcategories$Var2=="Livestock"])
shockcategories$Value[shockcategories$Var2=="Fisheries"]<- sum(shockcategories$Freq[shockcategories$Var2=="Fisheries"])
shockcategories$Prop <- shockcategories$Freq/ shockcategories$Value
